...
const NOISE_ENTITIES = new Set([
    ...
    'skip', 'returns', 'successfully', 'note', 'notes',
    // German
    'ich', 'du', 'er', 'sie', 'es', 'wir', 'ihr', 'mich', 'dich', 'sich',
    'mir', 'dir', 'mein', 'dein', 'sein', 'unser', 'euer',
    'der', 'die', 'das', 'den', 'dem', 'des', 'ein', 'eine', 'einer',
    'nur', 'über', 'aber', 'oder', 'und', 'wenn', 'weil', 'dass',
    'auch', 'noch', 'schon', 'wieder', 'mehr', 'viel', 'sehr',
    'hier', 'da', 'dort', 'wo', 'wie', 'was', 'wer', 'wann',
    'ja', 'nein', 'okay', 'bitte', 'danke', 'gerne', 'klar',
    'machen', 'haben', 'werden', 'können', 'sollen', 'müssen', 'wollen',
    'ändern', 'änder', 'gemacht', 'getan',
    // Common code noise
    'function', 'const', 'let', 'var', 'return', 'async', 'await', 'try', 'catch',
]);

/**
 * Strip markdown formatting artifacts from entity names.
 */
function stripMarkdown(name) {
    if (!name) return name;
    ...
    let cleaned = name
        ...
        .replace(/^[-•]\s+/, '')
        // Table pipes: | Name → Name
        .replace(/^\|\s*/, '')
        .replace(/\s*\|$/, '')
        // Trailing markdown chars: Name** → Name
        .replace(/[*_`]+$/, '')
...
function isValidEntityName(name) {
    ...
    // Check against noise list (case-insensitive)
    if (NOISE_ENTITIES.has(cleaned.toLowerCase())) return false;
    ...
    // Single lowercase word under 4 chars is almost always noise
    if (cleaned.length <= 3 && /^[a-z]+$/.test(cleaned)) return false;
    
    // Reject markdown artifacts
    if (/^[-•|*_`\[\]]/.test(cleaned)) return false;
    if (/[-•|*_`\[\]]$/.test(cleaned)) return false;
    
    // Reject combined entities (Daniel + Markus → should be separate)
    if (/\s+[+&]\s+/.test(cleaned)) return false;
    
    // Sentence fragments: if it starts lowercase and has no capitals, reject
    if (/^[a-z]/.test(cleaned) && !/[A-Z]/.test(cleaned) && cleaned.length < 6) return false;
    
    // Reject if it looks like a sentence fragment
    if (/^(?:the|this|that|it|is|was|has|had|are|were|can|will|do|did|not|but|and|or|so|if|then)\s/i.test(cleaned)) return false;
    
    // ── JUNK FILTERS (added to prevent tool output / code / instruction noise) ──
    
    // Starts with parenthesis — compromise.js artifact from German text
    if (/^\(/.test(cleaned)) return false;
    
    // Contains "successfully" or "_bytes" — tool output artifacts
    if (/successfully|_bytes|\d+_bytes/i.test(cleaned)) return false;
    
    // Looks like a file operation result: "md_successfully", "py_successfully"
    if (/^\w+_successfully/i.test(cleaned)) return false;
    
    // Too many underscores — likely a code identifier or compound key
    if ((cleaned.match(/_/g) || []).length >= 3) return false;
    
    // Sentence-length strings (5+ words) — not a real entity
    if (cleaned.split(/\s+/).length >= 5) return false;
    
    // Contains code-like patterns: file extensions, paths, brackets
    if (/\.\w{2,4}$/.test(cleaned) && !/\.(com|org|net|de|eu|io)$/i.test(cleaned)) return false;
    if (/[{}()\[\];=><]/.test(cleaned)) return false;
    
    // German filler that compromise misclassifies as entities
    if (/^(nur|bei|beim|über|unter|nach|vor|seit|bis|gegen|ohne|zwischen)\b/i.test(cleaned) && cleaned.split(/\s+/).length <= 2) return false;
    
    // Skip/heartbeat instruction artifacts
    if (/^(skip|heartbeat|HEARTBEAT)/i.test(cleaned)) return false;
...
function extractFromExchange({ messages, config, knownEntities }) {
    ...
    // Per-predicate thresholds override global threshold
    const globalThreshold = cfg.confidenceThreshold || 0.5;
    const predicateThresholds = cfg.predicateThresholds || {};
    const filteredTriples = triples.filter(t => {
        const threshold = predicateThresholds[t.predicate] ?? globalThreshold;
